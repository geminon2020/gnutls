...
static int write_sigalg(gnutls_sign_algorithm_t sigalg, uint8_t *out)
{
	...
	for (unsigned i = 0; ; i++) {
		algo = &algos[i];
		if (algo->sign_algo == sigalg || algo->sign_algo == GNUTLS_SIGN_UNKNOWN)
			break;
	}
 ...
 *   and %XXXX if the SCT's version is different than 1, as that's currently
